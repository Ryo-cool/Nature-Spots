<template>
  <v-card class="mt-10">
    ...
    >
      ...
      <v-toolbar-title class="ml-4">{{ $auth.user.name }}さん</v-toolbar-title>
        ...
        <v-row class="mx-2">
          <v-col v-for="s  in myReview" :key="s.id" cols="12" md="4" sm="6">
            <v-card>
              ...
              <nuxt-link :to="`/spots/${s.spot.id}`" class="text-decoration-none">
                <v-card-title class="pb-0">{{ s.spot.name}}</v-card-title>
              </nuxt-link>
              <v-card-text class="pb-1">
                <v-row>
                  <v-rating v-model="s.rating" readonly></v-rating>
                  <span class="grey--text body-1 mr-2 pt-2">
                    ({{ s.rating }})
                  </span>
                </v-row>
              </v-card-text>
              <v-card-title class="py-1">{{ s.title }}</v-card-title>
        ...
        <v-row>
          <v-col v-for="i  in reviews" :key="i.id" cols="12" md="4" sm="6">
            <v-card>
              <v-card-text>
                <v-row>
                  <v-col cols="2" class="py-0">
                    <nuxt-link :to="`/user/${i.user.id}`" class="text-decoration-none">
                    <v-avatar size=36> 
              ...
              <nuxt-link :to="`/spots/${i.spot.id}`" class="text-decoration-none">
                <v-card-title class="pb-0">{{ i.spot.name}}</v-card-title>
              </nuxt-link>
              <v-card-text class="pb-1">
                <v-row>
                  <v-rating v-model="i.rating" readonly></v-rating>
                  <span class="grey--text body-1 mr-2 pt-2">
                    ({{ i.rating }})
                  </span>
                </v-row>
      ...
      <v-tab-item>
        <v-card flat>
          <v-card-text v-for="a  in likeSpot" :key="a.id">
            <v-img :src="a.photo.url" :aspect-ratio="16/9" max-height="200" max-width="250" />
